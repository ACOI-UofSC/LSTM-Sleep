...
def train(feature_sets, output_dir, class_num=2, loso=True):
    ...
    loso : bool
        True  → leave-one-subject-out CV  (default).
        False → 10-fold CV.
    """
    ...
    # NOTE: attributed_classifier is intentionally created inside the fold loop below.
    # Each fold must start from freshly initialised weights. If a single model
    # instance were shared across folds its weights would carry over from fold N
    # into fold N+1, causing every fold after the first to plateau on epoch 1.
    classifier_name = 'LocalGlobalLSTM'
    classifier_kwargs = dict(
        feature_dim=len(feature_sets[0]),  # 4 for AGV (x,y,z,vm)
        local_steps=30,
        n_class=class_num,
    )
    ...
    for fold_idx, data_split in enumerate(data_splits):
        ...
        # A new LocalGlobalLSTM is instantiated each fold so weights are always
        # randomly initialised — never contaminated by a previous fold's training.
        attributed_classifier = AttributedClassifier(
            name=classifier_name,
            classifier=LocalGlobalLSTM(**classifier_kwargs),
        )
